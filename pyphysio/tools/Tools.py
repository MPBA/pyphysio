    ...
    * 'grid' Grid search
            ...
            if len(selected_maxs) != 0:
                ...
                for idx_max in selected_maxs:
                    ...
                    if weight == 'exp':
                        ...
                        driver_detrended = (driver_detrended - _np.mean(driver_detrended)) * exp_weigths + _np.mean(
                            driver_detrended)
                    elif weight == 'lin':
                        lin_weigths = _np.arange(1, 0, -1 / len(driver_detrended))
                        driver_detrended = (driver_detrended - _np.mean(driver_detrended)) * lin_weigths + _np.mean(
                            driver_detrended)
    ...
    def _loss_benedek(par_bat, signal, delta, min_T1, max_T1, min_T2, max_T2, weight):
        ...
        def phasic_estim_benedek(driver, delta):
            ...
            if len(i_peaks) == 0:
                # TODO Andrea: Do not use print, use cls.warn or cls.error (return something that can make the Algorithm call it)
        ...
        # TODO Andrea: Do not use print, use cls.warn or cls.error (return something that can make the Algorithm call it, if not for debug)
#        print(len(phasic))
