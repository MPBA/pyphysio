import numpy as _np
from scipy.signal import _gaussian, _deconvolve
from ..BaseEstimator import _Estimator
...
class BeatFromBP(_Estimator):
    ...
    def algorithm(cls, signal, params): # FIX others
        ...
        if method == 'dt': #(derivative gaussian)
            ...
            signal_f = IIRFilter(1.2*fmax, 5*fmax)(signal)
            
        # TODO: once implemented estimate_delta, pass fmax and fmax/3 (fsamp is in delta_f)
        deltas = tll.estimate_delta(signal_f, fsamp/fmax, 3*fsamp/fmax)*0.5 ### Tools
    ...
    def check_params(cls, params):
        if (not 'bpm_max' in params) | (params['bpm_max'] < 0):
            # action_param(params, BPM_MAX=0)
            pass            
        if (not 'method' in params) | (params['sigma'] not in ['lp', 'dt']) :
            # action_param(params, METHOD ='lp')
            pass
        if (not 'sigma' in params) | (params['sigma'] < 0):
...
class BeatFromECG(_Estimator):
    ...
    @classmethod
    def algorithm(cls, signal, bpm_max, delta, **delta_pars):
        ...
        if delta==None:
            delta = tll.estimate_delta(signal, **delta_pars) ## Tools
...
class BeatOutliers(_Estimator):
	...
	"""
	@classmethod
    def algorithm(cls, signal, cache, sensitivity, ibi_median, remove):
...
class BeatOptimizer(_Estimator):
    ...
    """
	@classmethod
	def algorithm(signal, B, cache, sensitivity, ibi_median):
		# check signal is IBI 
		if ibi_median == 0:
		...
		for i in range(len(starts)):
			...
			combinations = list(itertools.product([0, 1], repeat= i_sp - i_st -1))
...
class DriverEstim(_Estimator):
	...
	@classmethod
    def check_params(cls, params):
		if not 'par_bat' in params or len(params['par_bat']) != 2:
...
class PhasicEstim(_Estimator):
	...
	@classmethod
	def algorithm(cls, signal, delta, grid_size, i_pre_max, i_post_max):
		ZERO = 0
		...
		max_driv, tmp_ = tll.peakdet(driver, delta) # TODO: Tools
		...
		driver_no_peak = np.copy(driver) # TODO: check
